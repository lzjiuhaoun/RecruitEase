  ...
  <div class="content">
    <div class="content-left">
      <div class="left-search-item">
        <h4>岗位分类</h4>
        <a-tree :tree-data="contentData.cData" :selected-keys="contentData.selectedKeys" @select="onSelectClass"
    ...
    <div class="content-right">
      ...
      <a-spin :spinning="contentData.loading" style="min-height: 200px;">
        <div v-if="contentData.selectTabIndex <= 1" class="pc-thing-list flex-view">
        ...
        <div v-else class="pc-thing-list flex-view">
          <div class="sub-li" v-for="item in contentData.pageData" @click="handleDetail(item)">
            <a class="job-info" target="_blank">
              <div class="sub-li-top">
                <div class="sub-li-info">
              ...
              <p class="job-text">
                <span class="assign-line">{{ item.description }}</span>
              </p>
            </a>
          </div>
          <div v-if="contentData.pageData.length <= 0 && !contentData.loading" class="no-data" style="">暂无数据</div>
...
const onSelectClass = (selectedKeys) => {
  contentData.selectedKeys = selectedKeys
  console.log(contentData.selectedKeys[0])
  let index = contentData.selectTabIndex
  let sort = (index === 0 ? 'full_time' : index === 1 ? 'part_time' : 'job_guide')
  contentData.selectTagId = -1
  if (contentData.selectedKeys.length > 0) {
    getList(sort, contentData.selectTagId, getSelectedKey())
  }
}
const clickTag = (index) => {
  contentData.selectedKeys = []
  contentData.selectTagId = index
  let tabIndex = contentData.selectTabIndex
  let sort = (tabIndex === 0 ? 'full_time' : tabIndex === 1 ? 'part_time' : 'job_guide')
  getList(sort, contentData.selectTagId, getSelectedKey())
}
...
const selectTab = (index) => {
  ...
  getList(data['sort'], data['tag'], data['c'])
}
...
const getList = (sort, tag, c) => {
  console.log({sort: sort, tag: tag, c: c})
  getThingList({sort: sort, tag: tag, c: c});
}
