    ...
    <a-spin :spinning="loading" style="min-height: 200px;">
      ...
      <div class="edit-view">
        <div class="item flex-view">
          <div class="label not-required">封面</div>
        ...
        <div class="item flex-view">
          <div class="label required">地点</div>
          <div class="right-box">
            <input type="text" v-model="tData.form.location" placeholder="请输入" maxlength="100" class="input-dom web-input">
          </div>
        </div>
        <div class="item flex-view">
          <div class="label required">行业</div>
          <div class="right-box">
            <input type="text" v-model="tData.form.industry" placeholder="请输入" maxlength="100" class="input-dom web-input">
          </div>
        </div>
        <div class="item flex-view">
          <div class="label not-required">规模</div>
          <div class="right-box">
            <input type="text" v-model="tData.form.size" placeholder="请输入" maxlength="100" class="input-dom web-input">
          </div>
        </div>
        <div class="item flex-view">
          <div class="label required">企业微信</div>
          <div class="right-box">
            <input type="text" v-model="tData.form.wechat" placeholder="请输入" maxlength="100" class="input-dom web-input">
            <p class="tip">该微信号会在公司通过就职申请后发送给求职者</p>
          </div>
        </div>
        <div class="item flex-view">
          <div class="label not-required">简介</div>
          <div class="right-box">
...
const submit = () => {
  ...
  if (tData.form.industry) {
    formData.append('industry', tData.form.industry)
  } else {
    message.warn("请填写行业")
    return
  }
  if (tData.form.size) {
    formData.append('size', tData.form.size)
  }
  if (tData.form.wechat) {
    formData.append('wechat', tData.form.wechat)
  } else {
    message.warn("请填写企业微信")
    return
  }
