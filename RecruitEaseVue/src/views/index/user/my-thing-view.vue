          ...
          <template #bodyCell="{ text, record, index, column }">
            <template v-if="column.key === 'operation'">
              <span>
                ...
                <a-popconfirm title="确定删除?" ok-text="是" cancel-text="否" @confirm="confirmDelete(h)">
                  <a href="#">删除</a>
                </a-popconfirm>
              </span>
            </template>
          ...
          <div>
            <a-form ref="myform" :label-col="{ style: { width: '80px' } }" :model="modal.form" :rules="modal.rules">
              <a-row :gutter="24">
                ...
                <a-col span="8">
                  <a-form-item label="兼职" name="sort">
                    <a-select placeholder="请选择是否为兼职" allowClear v-model:value="modal.form.sort">
...
const modal = reactive({
  ...
  rules: {
    ...
    sort: [{ required: true, message: '请选择是否为兼职', trigger: 'change ' }]
  },
});
...
const handleEdit = (record: any) => {
  ...
  if (modal.form.sort) {
    modal.form.sort = modal.form.sort.toString()
  }
  if (modal.form.cover) {
...
const handleOk = () => {
  myform.value
    ...
    .then(() => {
      ...
      if (modal.form.sort !== undefined) {
        formData.append('sort', modal.form.sort);
      }
