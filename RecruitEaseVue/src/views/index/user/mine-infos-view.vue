<template>
  <div class="mine-infos-view">
    <div class="info-box flex-view">
      ...
      <div class="name-box">
        <h2 v-if="userInfo.nickname" class="nick">{{ userInfo.nickname }}</h2>
        <!-- <h2 v-if="userInfo.nickname" class="nick">{{ userInfo.nickname }}</h2>
        <h2 v-else class="nick">{{ userInfo.username }}</h2> -->
        <div class="age">
    ...
    <div v-if="userStore.user_role === '0'" class="order-box">
    ...
    <div class="order-box">
      ...
      <div class="list">
        <div v-if="userStore.user_role !== '0'" class="mine-item flex-view" @click="clickMenu('myPostView')">
          <img :src="SettingIconImage" alt="编辑资料">
          <span>我的投递</span>
        </div>
        <div v-if="userStore.user_role !== '0'" class="mine-item flex-view" @click="clickMenu('resumeEditView')">
          <img :src="SettingIconImage" alt="编辑资料">
          <span>我的简历</span>
        </div>
        ...
        <div class="mine-item flex-view" @click="clickMenu('consultView')">
          <img :src="SettingIconImage">
          <span>我的咨询</span>
        </div>
      </div>
...
import { userInfoUpdate } from '/@/store/update-info'
import { BASE_URL } from "/@/store/constants";
const userStore = useUserStore();
const router = useRouter();

watch(() => userInfoUpdate.value, (val) => {
  if (val === true) {
    console.log('need to be update~~~')
    getUserInfo()
    userInfoUpdate.value = false
  }
})
...
onMounted(() => {
  console.log("userId: " + userStore.user_id + "role: " + userStore.user_role)
  getCollectThingList()
  getWishThingList()
  getUserInfo()
})
...
const getUserInfo = () => {
  ...
  detailApi({ userId: userId }).then(res => {
    if (res.data.avatar) {
      userInfo.avatarURL = BASE_URL + '/api/staticfiles/avatar/' + res.data.avatar
    }
